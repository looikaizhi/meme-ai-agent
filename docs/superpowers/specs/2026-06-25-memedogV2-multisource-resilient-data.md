...
## 十四、落地状态(2026-06-25,已实现)

`src/memedogV2/sources/` 已落地:`base.py`(规范 Facts + `FIELD_PRIORITY` 优先级表)、`gmgn_source.py`(归一化 + 非 429 有界重试,修 H-1)、`rugcheck_source.py`、`helius_source.py`、`resolver.py`(优先级合并 + 失败容忍 + 每字段来源标注 + `momentum_unavailable`)。`hardfilter/facts_filter.py` 在规范 Facts 上跑红线;`harness/runner.py` 改由 `DataResolver` 取数(`read_facts` 一步,**源/解析失败 → FAILED 步,永不崩**,修 C-1);`__main__.py` 接三源。配置加 `gmgn.max_retries`。

**真实环境验证(实跑)**:
- 真实多源对用户给定的新币(`2kDX…bonk`)→ `lp_safe=True` **来源 rugcheck**(`lpLockedPct=100`,gmgn 同币报 `burn_status=none`)——多源修正了 gmgn 单源的 LP 误判;该币因真实零流动性($0.0002)正确 drop 在动量。
- `test_gate_resilience_real_fallback`(默认 suite 就跑):强制 RugCheck 失败 → 真实 gmgn 回退拿到权限,流程不崩。✓
- 真实发现:RugCheck `topHolders.pct` 对极新币可能 >100(失真)→ adapter 守护(>100 退 None,回退 gmgn/helius);Helius `getTokenLargestAccounts` 对大币 overload、且瞬时 http=000 → 仅作集中度垫底,失败即跳。

**仍延后**:`compliance.py`、完整 `replay.py`。
