...
本次改造目标:
1. **数据源统一到 GMGN**:用 `gmgn-cli`(GMGN OpenAPI 的确定性 CLI)接管安全/持仓/流动性取数,**删除 RugCheck 与 Helius**。
2. **HardFilter 只做客观红线**:用 `gmgn-cli` 三条命令的硬事实做排雷,产出极少数过闸候选。
3. **判断完全交给 LLM**:删除 ScoreEngine,不再有前置 0–100 分;由 Bull/Bear 分析师 + Judge(可用 gmgn-skills 主动取证)直接给出 `BULLISH/BEARISH/NEUTRAL` 与是否推荐。

## 2. 范围

**在范围内:**
- `gmgn-cli` 确定性封装(`clients/gmgn_cli.py`),含限速/缓存/429 退避。
- HardFilter 改造:三条 gmgn-cli 命令 + 客观红线规则。
- LLM 审计 workflow:证据采集 → Bull → Bear → Judge,经 `codex exec` 调起、用 gmgn-skills 取证。
- 数据契约更新(删 `TokenSnapshot`/`Score`,加 `HardFilterResult`/`EvidenceBundle`,`Signal` 加 `recommended`)。
- orchestrator 中段替换;PaperTrader/Dashboard/Alert 字段微调。
- 删除 `clients/rugcheck.py`、`clients/helius.py`、`enricher/`、`scoring/`。

**不在范围内(本次不做):**
- **Telegram bot 抓取**:系统入口直接接受 `(ca_address, lp_address)`。抓取层留接口占位,手动/测试可直接喂地址。
- Scanner/DexScreener 发现层:退出主路径(代码暂留仓库,主循环不再调用)。
...
**`clients/gmgn_cli.py`** —— `gmgn-cli` 子进程封装:
- 方法:`token_security(ca)`、`token_pool(ca)`、`token_info(ca)`,均 `--chain sol --address <CA> --raw`,解析 JSON。
- 自带:**客户端限速器**(token-bucket,复用 `clients/ratelimit.py`,默认保守 `~1 req/s` 可配)、**按地址结果缓存**(TTL)、**429 感知退避**(读 `reset_at`,封禁期间挂起到解封,**绝不冷却期重试**——重试会把封禁每次延长 5s)。
...
## 5. 数据契约变化
- **删除**:`models/snapshot.py`(`TokenSnapshot`)、`models/score.py`(`Score`)。
- **新增**:`HardFilterResult`(地址 + gmgn 事实 + 通过/红线/标记)、`EvidenceBundle`(LLM 证据字段)。
- **修改**:`Signal` 增加 `recommended: bool` 与 `evidence_refs`。
...
## 9. 删除清单
- `src/memedog/clients/rugcheck.py` + 其测试
- `src/memedog/clients/helius.py` + 其测试
- `src/memedog/enricher/` 整目录 + 测试
- `src/memedog/scoring/`(ScoreEngine)+ 测试
- `models/snapshot.py`、`models/score.py`
- Scanner/discovery:退出主路径(orchestrator 不再调用;代码暂留,后续清理)
...
2. 数据契约:`HardFilterResult` / `EvidenceBundle` / `Signal.recommended`(TDD)。
3. `clients/gmgn_cli.py`:限速 + 缓存 + 429 退避(mock 子进程 TDD)。
4. HardFilter 改造:三命令 + 红线规则(mock CLI TDD)。
5. 审计 workflow:证据采集 agent + bull/bear/judge + codex 调用层(mock codex TDD)。
6. 接回 orchestrator;AddressIntake 队列占位。
7. 删除旧模块(RugCheck/Helius/Enricher/ScoreEngine)。
8. Dashboard/Alert 字段微调。
